...
export default function SignupForm({ lang }: { lang: LangCode }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    if (!res.ok) return setError(data.error || t(lang, "signupFailed"));
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6">
      <form onSubmit={onSubmit} className="w-full max-w-[380px] bg-surface rounded-2xl shadow-card border border-border p-7">
        <h1 className="text-[22px] font-bold tracking-tight2 mb-6">{t(lang, "startFacility")}</h1>
        <label className="block text-[13px] font-medium text-text-secondary mb-1.5">{t(lang, "facilityName")}</label>
        ...
        <button disabled={loading} className="w-full bg-primary text-white rounded-full py-2.5 text-[14px] font-medium">
          {loading ? t(lang, "creating") : t(lang, "signUp")}
        </button>
        <div className="text-[13px] text-text-secondary mt-4 text-center">
          {t(lang, "haveAccount")} <Link href="/login" className="text-primary">{t(lang, "signIn")}</Link>
        </div>
      </form>
    </main>
  );
