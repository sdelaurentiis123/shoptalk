...
export function buildChunkContext(
  prevTranscript: { timeSeconds: number; text: string }[],
  prevEndSec: number,
): string {
  const tail = prevTranscript.slice(-30);
  const lines = tail.map((b) => `[${b.timeSeconds}s] ${b.text}`).join("\n");
  return `--- CONTEXT: Previous segment ended at ${prevEndSec}s ---\n${lines}\n--- END CONTEXT ---\n\nContinue from where the previous segment left off:`;
}
