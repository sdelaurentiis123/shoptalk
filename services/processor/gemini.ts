...
export function sanitizeTimestamps(
  ...
  const totalFromModel =
    coerceSeconds(out.totalSeconds, 24 * 60 * 60) ?? 0;
  const total =
    maxDuration ?? (totalFromModel > 0 ? totalFromModel : 24 * 60 * 60);
  const hasReliableTotal = maxDuration != null || totalFromModel > 0;

  let lastEnd = 0;
  const cleanedSteps = (out.steps ?? []).map((s) => {
    let start =
      coerceSeconds((s as any).startSeconds, total) ?? lastEnd;
    let end =
      coerceSeconds((s as any).endSeconds, total) ?? start + 5;
  ...
  return {
    ...out,
    totalSeconds: hasReliableTotal ? total : 0,
    steps: cleanedSteps,
  };
}
...
async function uploadLargeFileResumable(
  ...
  if (!sessionUrl)
    throw new Error("Gemini resumable start: no upload URL in response");
  ...
  if (!f?.name)
    throw new Error("Gemini resumable finalize: missing file metadata");
...
async function uploadAndWait(
  ...
  if (buf.length > RESUMABLE_THRESHOLD) {
    ...
    const tmpPath = join(
      tmpdir(),
      `shoptalk-${crypto.randomUUID()}-${safeName}`,
    );
  ...
  for (let i = 0; i < 150 && file.state !== "ACTIVE"; i++) {
    if (file.state === "FAILED")
      throw new Error(
        `Gemini file processing failed: ${JSON.stringify(file)}`,
      );
...
export async function processWithGemini(
  ...
  const response = useInline
    ...
    : await (async () => {
        const { uri, mimeType: uploadedMime } = await uploadAndWait(
          buf,
          mimeType,
          fileName,
        );
  ...
  return isSopPrompt
    ? sanitizeTimestamps(parsed, mimeType, opts?.maxDurationSec)
    : parsed;
}
